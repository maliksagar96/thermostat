module class_module
  implicit none
    ...
    real*8, parameter :: sigma = 1.0d0, mass = 1.0d0, temp = 9.0d0, eps = temp
    ...
    integer, parameter :: seed_x = 255, seed_y = 46, seed_z = 2588 !To check random number calling for differernt seeds
    ...
    real*8 :: f_rc = 4.0d0*eps*((12.0d0*(sigma**12)/(rc**13)) -(6.0d0*(sigma**6)/rc**7))
    real*8 :: u_rc = 4.0d0*eps*((sigma**12)/(rc**12) - (sigma**6)/(rc**6))
    real*8 :: KE_initial
...
contains
  !Defining all the funtions first then defining all the subroutines
    ...
    function velInit(arrayDim, seed) result(v)
      ...
      integer :: i, j = 0
      real*8 :: vc   !velocity constant
      vc  = dsqrt(3.0d0*temp)
	  call srand(seed)
	  
	  do i = 1, size(v)
        v(i) = 2.0d0* vc *(rand() - 0.50d0)
      end do
    ...
    function calcForce(xi, xj, r) result(f)
      ...
      xDiff = minDist(xi, xj)
      sig6 = (sigma**6)/(r**6)
      f = 6.0d0*sig6*4.0d0*eps*(2.0d0*sig6-1.0d0)*(xDiff)/(r**2) - f_rc*(xDiff)/abs(r)
    ...
    function calcPE(r) result(PE)
      ...
      real*8 :: PE, sig6
      sig6 = (sigma**6)/(r**6)
      PE = 4.0d0*eps*sig6*(sig6 - 1.0d0)! - (u_rc) + (r * f_rc) - (rc * f_rc)
    end function calcPE
    ...
    subroutine initVelocities()
      ...
      KE_initial = 0.0d0
      do i = 1, particles
        KE_initial = KE_initial + calcKE(velocity_x(i), velocity_y(i), velocity_z(i))
      end do
	...
	subroutine dataWrite() 
		implicit none
		integer :: xF = 21,yF = 22,zF = 23
		integer :: vx = 31,vy = 32,vz = 33
		integer :: fx = 41, fy = 42, fz = 43
		integer :: ke_init = 75
		integer :: i
		open(xF, file = 'x.dat')
		open(yF, file = 'y.dat')
		...
		open(fy, file = 'fy.dat')
		open(fz, file = 'fz.dat')
		open(ke_init, file = 'KE_initial.dat')
		
		do i = 1, particles
			write(xF, *) x(i)
			write(yF, *) y(i)
			write(zF, *) z(i)
			write(vx, *) velocity_x(i)
			write(vy, *) velocity_y(i)
			write(vz, *) velocity_z(i)
			write(fx, *) force_x(i)
			write(fy, *) force_y(i)
			write(fz, *) force_z(i)
		end do 
			
			write(ke_init, *) KE_initial
